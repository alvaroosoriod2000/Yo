...
game_state = {
    ...
    "donuts": 0,
    "events": [],
    "is_live": False  # NUEVO: Indicador de conexión
}
...
<head>
    ...
    <style>
        body {
            background-color: #2E8B57; /* Verde cancha de fútbol */
            color: white;
            ...
            height: 100vh;
            border: 10px solid white; /* Líneas de la cancha */
            position: relative;
        }
        h1 {
            color: #FFD700;
        ...
        /* NUEVO: Contenedor del bombillo de estado */
        .status-container {
            position: absolute;
            top: 20px;
            right: 20px;
        ...
        .status-light.active {
            background-color: #00FF00; /* Verde cuando está activo */
        ...
        .feed {
            ...
            font-size: 12px;
            line-height: 1.8;
            color: #00FF00; /* Letras verdes tipo terminal antigua */
        }
        .goal-animation {
...
<body>
    ...
    <div class="feed" id="eventFeed">
        <div>> Sistema iniciado. Esperando regalos...</div>
    </div>
    ...
    <script>
        ...
        // Esta función le pregunta a Python el estado del juego cada segundo
        async function fetchState() {
            try {
                const response = await fetch('/api/state');
                const data = await response.json();

                // NUEVO: Actualizar Bombillo de Estado
                ...
                // Animación de Gol si el puntaje cambió
                if (data.mexico_goals > lastGoalMx) {
                    document.getElementById('scoreDisplay').classList.add('goal-animation');
                    setTimeout(() => document.getElementById('scoreDisplay').classList.remove('goal-animation'), 3000);
                    lastGoalMx = data.mexico_goals;
                }
                if (data.argentina_goals > lastGoalAr) {
                    document.getElementById('scoreDisplay').classList.add('goal-animation');
                    setTimeout(() => document.getElementById('scoreDisplay').classList.remove('goal-animation'), 3000);
                    lastGoalAr = data.argentina_goals;
                }

                // Actualizar Feed de Eventos
                const feedDiv = document.getElementById('eventFeed');
                feedDiv.innerHTML = "";
        ...
        // Ejecutar cada 1 segundo (1000 ms)
...
@app.route('/api/state')
def get_state():
    # Devuelve el estado actual a la página web
    return jsonify(game_state)

def add_event(message):
    game_state["events"].insert(0, f"> {message}")
    if len(game_state["events"]) > 5:
        game_state["events"].pop()

# ==========================================
# 4. CONEXIÓN A TIKTOK LIVE (LÓGICA DEL JUEGO)
# ==========================================
def start_tiktok_client():
    # IMPORTANTE: CAMBIA ESTO POR TU NOMBRE DE USUARIO REAL
    TIKTOK_USERNAME = "@firu0123456789"

    # Creamos un nuevo loop de eventos para este hilo en segundo plano
    ...
    async def on_connect(event: ConnectEvent):
        game_state["is_live"] = True  # Enciende el foco verde
        print(f"Conectado a TikTok Live! Sala ID: {event.room_id}")
        add_event("Conectado a TikTok Live exitosamente.")

    @client.on(DisconnectEvent)
    async def on_disconnect(event: DisconnectEvent):
        game_state["is_live"] = False  # Apaga el foco (rojo)
...
if __name__ == '__main__':
    # 1. Arrancar la conexión a TikTok en un hilo en segundo plano
    tiktok_thread = threading.Thread(target=start_tiktok_client, daemon=True)
    tiktok_thread.start()

    # 2. Arrancar el servidor web Flask
